...
describe("StudyService", () => {
  ...
  describe("getReviewDeck", () => {
    it("should return an array of flashcards due for review", async () => {
      const mockFlashcards: FlashcardDto[] = [
        { id: "1", front: "Q1", back: "A1", leitner_box: 1, user_id: "user-1", next_review_at: new Date().toISOString(), created_at: "", updated_at: "", source: "manual" },
        { id: "2", front: "Q2", back: "A2", leitner_box: 2, user_id: "user-1", next_review_at: new Date().toISOString(), created_at: "", updated_at: "", source: "manual" },
      ];
  ...
  describe("gradeFlashcard", () => {
    it("should correctly upgrade a flashcard and set next review date on 'correct' outcome", async () => {
        ...
        mockSupabaseClient.single.mockResolvedValueOnce({ data: { ...currentFlashcard, ...updatedFlashcardData }, error: null });
    ...
    it("should throw an error if the flashcard to grade is not found", async () => {
        ...
        await expect(studyService.gradeFlashcard("non-existent-id", "user-1", "correct"))
            .rejects.toThrow("Flashcard not found or user does not have access.");
    });
     ...
     it("should throw an error if the update operation fails", async () => {
        ...
        await expect(studyService.gradeFlashcard("1", "user-1", "correct"))
            .rejects.toThrow("Could not update the flashcard.");
    });
  });
});
